...
class Tecnico{
    ...
    method getEspecialidad() = especialidad

    method esEspecialistaEn(familia){
        return especialidad == familia
    }
}
...
class Verificacion{
    ...
    method getInstrumento() = instrumento
    method getTecnico() = tecnico
    method getFecha() = fecha
}

object gestorVerificaciones{
    var verificaciones = []

    method puedeRevisar(instrumento, tecnico, fechaActual){
        if(!tecnico.esEspecialistaEn(instrumento.getFamilia())){
            return false
        }

        const ult = self.ultimaVerificacionDe(instrumento)
        if(ult == null){
            return true
        }

        const diff = calculadorDiferenciaFechas.calcular(ult.getFecha(), fechaActual)

        if(diff.getAnios() == 0 && diff.getMeses() == 0 && diff.getDias() < 7){
            return false
        }
    ...
    method afinarSegunTipo(instrumento){
        const nombre = instrumento.nombre()

        if(nombre == "FenderStratocaster"){
            return
        }
        ...
        if(nombre == "Bechstein"){
            if(instrumento.getLargo != null && instrumento.getAncho != null){
                instrumento.largo = 8
                instrumento.ancho = 4
            }
            return
        ...
        if(nombre == "Stagg"){
            instrumento.tremolo = 0
            return
        }
    }

   method registrarVerificacion(instrumento, tecnico, fechaActual){
        verificaciones.add(new Verificacion(instrumento = instrumento, tecnico = tecnico, fecha = fechaActual))

        if(instrumento.afinaciones != null){
            instrumento.afinaciones.add(fechaActual)
        }
    }

    method revisar(instrumento, tecnico, fechaActual){
        if(!self.puedeRevisar(instrumento, tecnico, fechaActual)){
            return false
        }

        self.afinarSegunTipo(instrumento)

        self.registrarVerificacion(instrumento, tecnico, fechaActual)

        return true
    }

    method verificacionesDe(instrumento){
        var res = []
        verificaciones.forEach({v =>
            if(v.getInstrumento() == instrumento){
                res.add(v)
            }
        })
        return res
    ...
    method ultimaVerificacionDe(instrumento){
        var todas = self.verificacionesDe(instrumento)
        if(todas.size() == 0){
            return null
        }
        return todas.last()
    ...
    method revisionesRecientesDe(instrumento, fechaActual){
        var rec = []
        self.verificacionesDe(instrumento).forEach({v =>
            const diferencia = calculadorDiferenciaFechas.calcular(v.getFecha(), fechaActual)
            if(diferencia.getAnios() == 0 && diferencia.getMeses() < 2){
                rec.add(v)
            }
        })
        return rec
